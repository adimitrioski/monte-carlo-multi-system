...
inline float calculate_mean(
	...
	float sum = thrust::reduce(thrust::cpp::par, h_vector.begin(), h_vector.end());
...
inline float get_min(
	...
	std::vector<float>::iterator min_iterator = thrust::min_element(thrust::cpp::par, h_ending_values.begin(), h_ending_values.end());
	return *min_iterator;
}
...
inline float get_max(
	...
	std::vector<float>::iterator max_iterator = thrust::max_element(thrust::cpp::par, h_ending_values.begin(), h_ending_values.end());
	return *max_iterator;
};
...
MonteCarloResult cpu_cpp_run_monte_carlo_simulation(
	...
	cpu_cpp_sparse_histogram(h_ending_values, h_histogram_values, h_histogram_counts);
